...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            if (command.options?.adminRole) {
                const adminRoleIDs = config.users?.adminRole || ["1246480261965676675"]; // Ambil daftar role ID dari config
                ...
                if (adminRoleIDs.length > 0) {
                    ...
                    if (!hasAdminRole) {
                        await interaction.reply({
                            content:
                                config.messageSettings.adminMessage && config.messageSettings.adminMessage.trim() !== ""
                                    ? config.messageSettings.adminMessage
                                    : "You are not authorized to use this command",
                            ephemeral: true,
                        });
                        return;
                    ...
                    await interaction.reply({
                        content:
                            config.messageSettings.missingDevIDsMessage && config.messageSettings.missingDevIDsMessage.trim() !== ""
                                ? config.messageSettings.missingDevIDsMessage
                                : "This is an Official Admin command of Noverra Roleplay only.",
                        ephemeral: true,
                    });
                    return;
                }
